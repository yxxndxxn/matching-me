...
import { currentUserProfile, getLifestyleTags, getMajorCategoryLabel, type UserProfile } from "@/lib/types"
...
function MenuItem({ icon, label, onClick, badge }: MenuItemProps) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center h-14 px-4 hover:bg-muted/30 transition-colors"
    >
      ...
      {badge !== undefined && badge > 0 && (
        <span className="mr-2 px-2 py-0.5 text-xs font-medium bg-primary/10 text-primary rounded-full">
          {badge}
        </span>
      )}
...
function PlaceholderPage({ 
  ...
  return (
    <div className="min-h-screen pb-24 bg-background">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 pt-12 pb-4 border-b border-border/50">
        <button 
          onClick={onBack}
      ...
      <div className="flex flex-col items-center justify-center px-6 py-20">
        ...
        <p className="text-sm text-muted-foreground text-center max-w-xs">
          이 페이지는 현재 준비 중입니다. 곧 업데이트될 예정이에요.
        </p>
      </div>
    </div>
  )
...
// Saved Roommates Page with clickable cards
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {profile.university} · {getMajorCategoryLabel(profile.majorCategory)} · {profile.grade}
                  </p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-xs text-muted-foreground">{profile.dormitory}</span>
...
export function ProfileView({ 
  ...
  const handleViewSavedProfile = (profileToView: UserProfile) => {
  ...
  // If viewing a saved profile's detail
  if (selectedProfile) {
    return (
      <AnimatePresence mode="wait">
        <motion.div
          key="saved-profile-detail"
  ...
  if (currentPage === "editProfile") {
    return <PlaceholderPage title="프로필 수정" onBack={handleBack} />
  }
  if (currentPage === "notifications") {
    return <PlaceholderPage title="알림 설정" onBack={handleBack} />
  }
  if (currentPage === "settings") {
    return <PlaceholderPage title="설정" onBack={handleBack} />
  }
  if (currentPage === "privacy") {
    return <PlaceholderPage title="개인정보 보호" onBack={handleBack} />
  }
  if (currentPage === "help") {
    return <PlaceholderPage title="도움말 및 지원" onBack={handleBack} />
  }
  if (currentPage === "savedRoommates") {
    return (
      <SavedRoommatesPage 
        onBack={handleBack} 
        savedProfiles={savedProfiles}
        onViewProfile={handleViewSavedProfile}
      />
    )
  }
  ...
  return (
    <div className="min-h-screen pb-24 bg-background">
      ...
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-background px-4 pt-12 pb-6">
        ...
        <div className="flex flex-col items-center text-center">
          <div className="relative">
            ...
            <Button
              ...
              className="absolute -bottom-1 -right-1 size-8 rounded-full shadow-md border border-border"
            >
      ...
      <main className="px-4 py-4 space-y-4">
            ...
            <div className="flex items-center gap-3 p-3.5 bg-muted/50 rounded-xl">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">
                  {profile.chronotype === "morning" ? "아침형" : "밤형"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3.5 bg-muted/50 rounded-xl">
              ...
              <div>
                <p className="text-[10px] text-muted-foreground uppercase">성격 유형</p>
                <p className="text-sm font-medium text-foreground">
                  {profile.personality === "extrovert" ? "외향형" : "내향형"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3.5 bg-muted/50 rounded-xl">
        ...
        <div className="bg-card rounded-2xl border border-border/50 overflow-hidden">
          ...
          <MenuItem 
            ...
            label="내가 찜한 룸메이트" 
            onClick={() => setCurrentPage("savedRoommates")}
        ...
        <button 
          onClick={onLogout}
          className="w-full flex items-center justify-center gap-2 h-14 text-destructive hover:bg-destructive/5 rounded-2xl transition-colors border border-border/50 bg-card"
        >
