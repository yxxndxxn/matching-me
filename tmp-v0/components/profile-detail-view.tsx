...
// Elegant circular progress with thin stroke and soft indigo gradient
...
export function ProfileDetailView({
  ...
  const handleRevealClick = () => {
    if (dailyRevealsRemaining === 0) {
      toast.error("오늘의 연락처 공개 횟수를 모두 사용했습니다.", {
  ...
  const handleConfirmReveal = () => {
    ...
    toast.success("연락처가 공개되었습니다!", {
      description: "마이페이지 찜 목록에서 다시 확인할 수 있어요.",
    })
  }
  ...
  const handleCopy = async (text: string, label: string) => {
    ...
    // Auto-close contact card after 2 seconds
    setTimeout(() => {
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border/50">
        <div className="flex items-center gap-3 px-4 h-14">
          <button
            onClick={onBack}
            className="flex items-center justify-center size-10 -ml-2 rounded-full hover:bg-secondary transition-colors"
          >
      ...
      <section className="px-6 pt-8 pb-6">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-1.5 mt-1 text-sm text-muted-foreground">
            ...
            <span>
              {getDormitoryLabel(profile.dormitory)} · {profile.grade}
            </span>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-5">
            {lifestyleTags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-medium bg-secondary text-secondary-foreground"
              >
                {tag}
              </span>
            ))}
          ...
          {!contactRevealed ? (
            ...
            >
              <Button
                ...
                className={cn(
                  "w-full h-14 rounded-2xl text-base font-semibold gap-2 transition-all duration-300",
              ...
              <div className="bg-card rounded-2xl border border-indigo-500/20 p-5 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    variant="default"
                    size="sm"
                    onClick={() => handleCopy(profile.phone, "전화번호")}
                    className="h-10 px-4 rounded-xl text-xs font-semibold gap-2 bg-indigo-500 hover:bg-indigo-600"
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    variant="default"
                    size="sm"
                    onClick={() => handleCopy(profile.kakaoId, "카카오톡 ID")}
                    className="h-10 px-4 rounded-xl text-xs font-semibold gap-2 bg-indigo-500 hover:bg-indigo-600"
                  >
                    <Copy className="size-4" />
                    복사
                ...
                {/* Etiquette Notice */}
                <p className="text-center text-xs text-muted-foreground pt-3 border-t border-border/30">
                  연락 시 예의를 갖춰 주세요!
                </p>
              </div>
            </motion.div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-medium text-foreground">
                남은 횟수: {dailyRevealsRemaining}/{maxDailyReveals}
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
